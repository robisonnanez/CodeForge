            ...
            <div className="space-y-4">
                <section className="atlantis-card atlantis-dark-card p-4">
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                        <Dropdown
                            value={user.id}
                            options={page.props.users.map((entry) => ({
                                label: `${entry.name} (${entry.email})`,
                                value: entry.id,
                            }))}
                            onChange={(event) => setUserId(Number(event.value))}
                        ...
                        <Dropdown
                            ...
                            onChange={(event) =>
                                router.post(
                                    `/admin/users-permissions/${user.id}/role`,
                                    { role: event.value || null },
                                )
                            }
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    {page.props.modules.map((module) => {
                        ...
                        const moduleEnabled =
                            selectedDirect.has(modulePermission) ||
                            rolePermissionNames.has(modulePermission);
                            ...
                            >
                                ...
                                <div className="space-y-2">
                                    {module.menus.map((menu) => {
                                        ...
                                        const isDirect = permissionName
                                            ? selectedDirect.has(permissionName)
                                            : false;
                                        const isInherited = permissionName
                                            ? rolePermissionNames.has(
                                                  permissionName,
                                              ) && !isDirect
                                            : false;
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {isDirect && (
                                                    <small className="text-[11px] text-emerald-400">
                                                        Directo
                                                    </small>
                                                )}
                                            </label>
